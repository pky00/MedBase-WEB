...
@Component({
  selector: 'app-equipment-category-form',
...
export class CategoryFormComponent implements OnInit {
  ...
  loadCategory(): void {
    ...
    this.api.get<Category>(`${API.EQUIPMENT_CATEGORIES}/${this.categoryId}`).subscribe({
      next: (category) => {
      ...
      error: () => {
        ...
        this.router.navigate([ROUTES.EQUIPMENT_CATEGORIES]);
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEdit()) {
      ...
      this.api.put<Category>(`${API.EQUIPMENT_CATEGORIES}/${this.categoryId}`, data).subscribe({
        next: () => {
      ...
      this.api.post<Category>(API.EQUIPMENT_CATEGORIES, data).subscribe({
        next: () => {
          this.saving.set(false);
