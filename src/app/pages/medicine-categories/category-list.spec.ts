...
describe('MedicineCategoryListComponent', () => {
  ...
  const mockResponse = {
    items: [{ id: 1, name: 'Antibiotics', description: 'Antibiotic medicines' }],
    total: 1,
    page: 1,
    size: 10,
    pages: 1,
  ...
  it('should load categories on init', () => {
    expect(api.getList).toHaveBeenCalledWith('medicine-categories', expect.objectContaining({ page: 1 }));
  ...
  it('should show error on load failure', () => {
    ...
    expect(notification.error).toHaveBeenCalledWith('Failed to load medicine categories.');
  ...
  describe('onAction', () => {
    it('should navigate to edit on edit action', () => {
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['/medicine-categories', 1, 'edit']);
    });

    it('should open delete modal on delete action', () => {
      component.onAction({ action: 'delete', item: { id: 1, name: 'Antibiotics' } });
      expect(component.deleteModalOpen()).toBe(true);
      expect(component.categoryToDelete()).toEqual({ id: 1, name: 'Antibiotics' });
    });
  });
  ...
  it('should navigate to new category page', () => {
    ...
    expect(navigateSpy).toHaveBeenCalledWith(['/medicine-categories', 'new']);
  });

  describe('delete', () => {
    it('should delete category and reload', () => {
      component.categoryToDelete.set({ id: 1, name: 'Antibiotics' });
      component.confirmDelete();

      expect(api.delete).toHaveBeenCalledWith('medicine-categories/1');
      expect(notification.success).toHaveBeenCalledWith('Medicine category deleted successfully.');
    ...
    it('should show error on delete failure', () => {
      ...
      expect(notification.error).toHaveBeenCalledWith('Failed to delete category. It may have linked medicines.');
