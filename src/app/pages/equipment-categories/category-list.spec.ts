...
describe('EquipmentCategoryListComponent', () => {
  ...
  const mockResponse = {
    items: [{ id: 1, name: 'Surgical', description: 'Surgical equipment' }],
    total: 1,
    page: 1,
    size: 10,
    pages: 1,
  ...
  it('should load categories on init', () => {
    expect(api.getList).toHaveBeenCalledWith('equipment-categories', expect.objectContaining({ page: 1 }));
  ...
  it('should show error on load failure', () => {
    ...
    expect(notification.error).toHaveBeenCalledWith('Failed to load equipment categories.');
  ...
  describe('onAction', () => {
    it('should navigate to edit on edit action', () => {
      ...
      expect(navigateSpy).toHaveBeenCalledWith(['/equipment-categories', 1, 'edit']);
    });

    it('should open delete modal on delete action', () => {
      component.onAction({ action: 'delete', item: { id: 1, name: 'Surgical' } });
      expect(component.deleteModalOpen()).toBe(true);
      expect(component.categoryToDelete()).toEqual({ id: 1, name: 'Surgical' });
    });
  });
  ...
  it('should navigate to new category page', () => {
    ...
    expect(navigateSpy).toHaveBeenCalledWith(['/equipment-categories', 'new']);
  });

  describe('delete', () => {
    it('should delete category and reload', () => {
      component.categoryToDelete.set({ id: 1, name: 'Surgical' });
      component.confirmDelete();

      expect(api.delete).toHaveBeenCalledWith('equipment-categories/1');
      expect(notification.success).toHaveBeenCalledWith('Equipment category deleted successfully.');
    ...
    it('should show error on delete failure', () => {
      ...
      expect(notification.error).toHaveBeenCalledWith('Failed to delete category. It may have linked equipment.');
