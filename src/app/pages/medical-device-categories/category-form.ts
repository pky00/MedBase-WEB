...
@Component({
  selector: 'app-medical-device-category-form',
...
export class CategoryFormComponent implements OnInit {
  ...
  loadCategory(): void {
    ...
    this.api.get<Category>(`${API.MEDICAL_DEVICE_CATEGORIES}/${this.categoryId}`).subscribe({
      next: (category) => {
      ...
      error: () => {
        ...
        this.router.navigate([ROUTES.MEDICAL_DEVICE_CATEGORIES]);
      },
    });
  }
  ...
  onSubmit(): void {
    ...
    if (this.isEdit()) {
      ...
      this.api.put<Category>(`${API.MEDICAL_DEVICE_CATEGORIES}/${this.categoryId}`, data).subscribe({
        next: () => {
      ...
      this.api.post<Category>(API.MEDICAL_DEVICE_CATEGORIES, data).subscribe({
        next: () => {
          this.saving.set(false);
