...
@Component({
  selector: 'app-medical-device-category-list',
...
export class CategoryListComponent implements OnInit {
  ...
  loadCategories(): void {
    ...
    this.api.getList<Category>(API.MEDICAL_DEVICE_CATEGORIES, params).subscribe({
      ...
      error: () => {
        this.table.loading.set(false);
        this.notification.error('Failed to load medical device categories.');
      },
    });
  }
  ...
  onAction(event: { action: string; item: Record<string, unknown> }): void {
    ...
    switch (event.action) {
      case 'edit':
        this.router.navigate([ROUTES.MEDICAL_DEVICE_CATEGORIES, id, 'edit']);
        break;
      case 'delete':
  ...
  createCategory(): void {
    this.router.navigate([ROUTES.MEDICAL_DEVICE_CATEGORIES, 'new']);
  }
  ...
  confirmDelete(): void {
    ...
    this.api.delete(`${API.MEDICAL_DEVICE_CATEGORIES}/${category['id']}`).subscribe({
      next: () => {
        this.deleting.set(false);
        this.deleteModalOpen.set(false);
        this.categoryToDelete.set(null);
        this.notification.success('Medical device category deleted successfully.');
        this.loadCategories();
      },
      error: () => {
        this.deleting.set(false);
        this.notification.error('Failed to delete category. It may have linked medical devices.');
      },
    });
  }
